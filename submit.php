...
$stmt->bindParam(':job', $selectedJob); 
$stmt->bindParam(':employee', $selectedEmployee); 

// Execute the statement
if ($stmt->execute()) {
    // Generate receipt content
    $receiptContent = "
        <h1>Official receipt of Handyman Services | Home Repair</h1>
        <hr>
        ...
        <p><strong>Job:</strong> {$_POST['job']}</p>
        <p><strong>Employee:</strong> {$_POST['employee']}</p>
        <hr>
        <br>
        <br>
        <br>
        ...
        <br>
        <HR>
        <h1>THIS RECEIPT WILL NOT BE VALID IF NOT SEALED</h1>
    ";
